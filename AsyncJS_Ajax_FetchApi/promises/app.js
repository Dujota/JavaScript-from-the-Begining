const posts = [{ title: 'Post One', body: 'This is post one' }, { title: 'Post Two', body: 'This is post one' }];

function createPost(post) {
  return new Promise(function(resolve, reject) {
    // construct the new promise, pass in a function that takes two arguments, resolve is what to do when async done, reject is for any error handling.
    ...
    setTimeout(function() {
      ...
      const error = false; // mimic an error on the server

      if (!error) {
        // if no error then invoke the resolve
        resolve();
      } else {
        // else show the error in condole
